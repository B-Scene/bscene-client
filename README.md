...
- **Firebase Cloud Messaging (FCM)**  
  브라우저 푸시 알림을 위한 Google의 클라우드 메시징 서비스입니다.  
  B:Scene에서는 서비스 워커 등록과 알림 권한 요청, FCM 토큰 발급 및 서버 등록, 포그라운드/백그라운드 알림 수신, 알림 클릭 시 관련 화면으로 이동하는 딥링크 처리에 사용했습니다.
...
- Firebase Cloud Messaging 기반 웹 푸시 알림 구현 (권한 요청, 토큰 등록/갱신, 알림 클릭 시 딥링크 이동, 읽음 상태 처리)

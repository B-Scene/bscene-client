...
const formatDeadlineAt = (deadlineAt?: string) => {
  ...
  if (!dateValue || !timeValue) return deadlineAt;

  const [year, month, day] = dateValue.split("-").map(Number);
  if (!year || !month || !day) return `${dateValue} ${timeValue} 마감`;
...
export const useSessionRecruitmentDetail = ({
  ...
  const canDelete = detail?.isMine ?? post.isMine ?? false;

  const infoRows = [
    { label: "파트", value: detail?.part ?? post.tags[0] ?? "-" },
  ...
  const closeDeleteModal = () => {
    if (!deleteRecruitmentMutation.isPending) setIsDeleteModalOpen(false);
  };

  const confirmDelete = async () => {
    if (deleteRecruitmentMutation.isPending) return;
    setDeleteErrorMessage("");
  ...
  const sendMessage = () => {
    navigate(`/band/session/messages/${sessionRecruitmentId}`, {
      state: {
        senderName: bandName,
  ...
  return {
    ...
    isDetailLoaded: !detailQuery.isLoading && !detailQuery.isError,
    openDeleteModal,
    closeDeleteModal,
    confirmDelete,
    openApplicationModal: () => setIsApplicationModalOpen(true),

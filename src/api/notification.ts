...
const unwrapNotificationResult = <T>(response: NotificationApiResponse<T>) => {
...
export const getNotifications = async ({
  cursor,
  size = 20,
}: GetNotificationsParams): Promise<NotificationsPageResponse> => {
  const { data } = await axiosInstance.get<
    NotificationApiResponse<NotificationsPageResponse>
  >("/notifications", {
    params: {
      cursor,
      size,
  ...
  return unwrapNotificationResult(data);
};
...
export const getNotificationSettings = async ({
  ...
  const { data } = await axiosInstance.get<
    NotificationApiResponse<NotificationSettingsResponse>
...
export const updateNotificationSetting = async ({
  ...
  const { data } = await axiosInstance.patch<NotificationApiResponse<null>>(
    "/notifications/settings",
    {
      settingType,
      enabled,
...
export const registerPushToken = async (body: unknown) => {
  const { data } = await axiosInstance.post<NotificationApiResponse<null>>(
...
export const deletePushToken = async (body?: unknown) => {
  const { data } = await axiosInstance.delete<NotificationApiResponse<null>>(
    "/notifications/push-token",
    {
      data: body,
    },
